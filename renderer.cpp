...
#include "clocker.cpp"
		...
		class Renderer {
		private:
			HINSTANCE WindowINST = NULL;   //the instance of the window
			HWND WindowHandle = NULL;      //the handle of the window
			HDC WindowDC = NULL;           //Device Context of the window, used for drawing
		...
		public:
			...
			int drawLine(int x1, int y1, int x2, int y2, int color) {
				...
				if (x1 != x2) {
					m = (double)(y2 - y1) / (x2 - x1);		//simple arithmetics, straightforward int to double casting and back
					b = (double)y1 - (m * x1);
			...
			int fillcoordRect(int x1, int y1, int x2, int y2, int color) {


				int xrbound = x2, xlbound = x1, ytbound = y1, ybbound = y2;
				if (y1 < 0)
					ytbound = 0;
				if (x1 < 0)
					xlbound = 0;
				...
				for (int y = ytbound; y < ybbound; y++) {
					for (int x = xlbound; x < xrbound; x++) {
						((unsigned int*)content)[y * cwidth + x ] = color;
						((unsigned int*)content)[y * cwidth + x] = color;
					}
				}
			...
			int fillRect(int x1, int y1, int x2, int y2, int color) {


				int xrbound = x1+x2, xlbound = x1, ytbound = y1, ybbound = y1+y2;
				if (y1 < 0)
					ytbound = 0;
				if (x1 < 0)
					xlbound = 0;
			...
			int drawRect(int x1, int y1, int x2, int y2, int color) {
				...
				for (int y = ytbound; y < ybbound; y++) {
					...
					((unsigned int*)content)[y * cwidth + xlbound] = color;
					((unsigned int*)content)[y * cwidth + xrbound] = color;
				}


				for (int x = xlbound; x < xrbound; x++) {

					((unsigned int*)content)[ytbound * cwidth + x] = color;
					((unsigned int*)content)[ybbound * cwidth + x] = color;
				}
				return 0;
